...
export default function AdminHomepage() {
  ...
  const { data: banners = [], isLoading } = useQuery({
    ...
    queryFn: homepageApi.adminListBanners,
  });
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, d }: { id: string; d: Partial<typeof empty> }) =>
      homepageApi.updateBanner(id, d as any),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-banners"] });
  ...
  const toggleMut = useMutation({
    mutationFn: ({ id, is_active }: { id: string; is_active: boolean }) =>
      homepageApi.updateBanner(id, { is_active } as any),
  ...
  return (
    <AdminLayout
      ...
      subtitle="Manage banners and featured sections"
      actions={
        showForm ? (
      ...
      {showForm ? (
          ...
          <div className="border border-border p-6 space-y-4 bg-background">
            <label className="block">
              ...
              <input
                ...
                className={inp}
                placeholder="Premium Luxury Sarees"
              />
            </label>
            <label className="block">
              ...
              <input
                ...
                className={inp}
                placeholder="The Vivah Edit · AW26"
              />
            </label>
            <label className="block">
        ...
        <div className="py-20 text-center border border-dashed border-border">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            Add a hero banner to display on the homepage.
          </p>
          <button
            onClick={() => setShowForm(true)}
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {banners.map((b: HomepageBanner) => (
            <div
              key={b.id}
              className={`border bg-background ${!b.is_active ? "opacity-50" : "border-border"}`}
            >
              {b.image ? (
                <img src={b.image} className="h-40 w-full object-cover" />
              ) : (
                <div className="h-40 bg-champagne/20 flex items-center justify-center">
              ...
              <div className="p-4">
                ...
                {b.subtitle && (
                  <p className="text-xs text-muted-foreground italic mt-1">{b.subtitle}</p>
                )}
                {b.cta_text && (
