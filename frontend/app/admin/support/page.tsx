          ...
          <div className="space-y-4">
            {filtered.map((ticket: SupportTicket) => {
              ...
              return (
                <div
                  key={ticket.id}
                  className="bg-background rounded-lg border border-border shadow-sm hover:shadow-md transition-all overflow-hidden"
                >
                  <div
                  ...
                  >
                    <div className="space-y-2">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-mono text-[10px] text-muted-foreground uppercase bg-muted/40 px-2 py-0.5 rounded">
                          {ticket.ticket_number}
                        </span>
                        <span
                          className={`border px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wider rounded-sm ${statusColor}`}
                        >
                          {ticket.status.replace("_", " ")}
                        </span>
                        <span className="text-[9px] font-medium uppercase tracking-wider text-muted-foreground border border-border px-2 py-0.5 rounded-sm bg-muted/20">
                          ...
                          <div
                            ...
                            className={`p-4 rounded-lg text-sm shadow-sm ${
                              msg.sender_type === "customer"
                                ...
                                : "bg-gold/5 border border-gold/20 ml-12"
                            }`}
                          >
                      ...
                      <div className="border-t border-border pt-4">
                        ...
                        <div className="flex gap-2">
                          <textarea
                            ...
                            rows={3}
                            className="flex-1 border border-border bg-background px-3 py-2.5 text-sm focus:outline-none focus:border-foreground resize-none"
                          />
                          <button
                            onClick={() => {
