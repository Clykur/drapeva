...
export default function AdminReviews() {
  ...
  const filtered = reviews.filter((r: Review) => {
    const matchSearch =
      ...
      (r.product as any)?.name?.toLowerCase().includes(search.toLowerCase());
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {filtered.map((r: Review) => (
              <div
                key={r.id}
                className={`border bg-background p-5 transition-all ${!r.is_approved ? "border-amber-200 bg-amber-50/30" : "border-border"}`}
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      {!r.is_approved && (
                        <span className="text-[9px] uppercase tracking-widest bg-amber-100 text-amber-700 px-1.5 py-0.5 border border-amber-200">
                          Pending
                        </span>
                      )}
                      {r.is_approved && (
                        <span className="text-[9px] uppercase tracking-widest bg-emerald-50 text-emerald-600 px-1.5 py-0.5 border border-emerald-200">
                          Approved
                        </span>
                      )}
                    </div>
