...
export class EmailService {
  static async sendEmail(to: string, subject: string, html: string) {
    ...
    try {
      const data = await resend!.emails.send({
        from: "Maaya Couture <atelier@maayacouture.com>",
        to: [to],
        subject,
        html,
      });
  ...
  static async sendWelcomeEmail(to: string, name: string) {
    const html = `
      <div style="font-family: 'Playfair Display', Georgia, serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #faf9f6; color: #1a1612; border: 1px solid #e2dcd0;">
        <h1 style="text-align: center; letter-spacing: 0.15em; font-weight: 400; text-transform: uppercase;">MAAYA</h1>
        <p style="text-align: center; font-size: 0.75rem; letter-spacing: 0.25em; text-transform: uppercase; color: #8c7853; margin-top: -10px;">Couture</p>
        <hr style="border: 0; border-top: 1px solid #e2dcd0; margin: 30px 0;" />
        <p>Dear ${name},</p>
        <p>Welcome to the Maaya Couture atelier. Your account has been successfully registered.</p>
        <p>Discover our heirloom Banarasi weaves, custom lehengas, and bridal fittings designed to celebrate your most precious moments.</p>
        ...
        <p style="font-size: 0.85rem; color: #8c7853; line-height: 1.6;">Warmest regards,<br/>The Maaya Concierge Team</p>
      </div>
    `;
    return this.sendEmail(to, "Welcome to Maaya Couture Atelier", html);
  }
  ...
  static async sendOrderConfirmation(to: string, name: string, orderId: string, total: number) {
    const html = `
      <div style="font-family: 'Playfair Display', Georgia, serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #faf9f6; color: #1a1612; border: 1px solid #e2dcd0;">
        ...
        <p style="font-size: 0.85rem; color: #8c7853; line-height: 1.6;">With compliments,<br/>The Maaya Concierge Team</p>
      </div>
    `;
    return this.sendEmail(to, `Order Confirmation #${orderId} — Maaya Couture`, html);
  }
  ...
  static async sendAppointmentConfirmation(
    ...
    `;
    return this.sendEmail(to, "Atelier Consultation Scheduled — Maaya Couture", html);
  }
}
